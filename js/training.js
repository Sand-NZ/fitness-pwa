...
Training.init = function() {
  ...
  this._targetReps = null;
  this._accumulatedReps = 0;
  Timer.init({});
};
...
Training.recordSet = function(formData) {
  ...
  // 递减组：累计次数并标记组类型
  if (this._targetReps != null) {
    const isDrop = document.getElementById('drop-set-checkbox')?.checked || false;
    set._dropGroup = isDrop ? 1 : 0;
    const reps = parseInt(set.reps) || 0;
    this._accumulatedReps += reps;
  }
...
Training._switchExercise = function() {
  ...
  this._targetReps = null; this._accumulatedReps = 0;
  this.renderPage();
};
...
Training._saveAndEnd = function() {
  ...
  this._completedExercises = []; this._targetReps = null; this._accumulatedReps = 0; Timer.stop();
  localStorage.removeItem('fitness_pending_training');
...
// ---------- 设置目标次数 ----------
Training._setTargetReps = function() {
  const input = document.getElementById('target-reps-input');
  if (!input) return;
  const v = parseInt(input.value);
  if (v > 0) {
    this._targetReps = v;
    this._accumulatedReps = 0;
    App.showToast(`🎯 目标设为 ${v} 次`, 'info');
  } else {
    this._targetReps = null;
    this._accumulatedReps = 0;
  }
...
Training.autoSave = function() {
  ...
  localStorage.setItem('fitness_pending_training', JSON.stringify({
    ...
    _targetReps: this._targetReps, _accumulatedReps: this._accumulatedReps,
...
Training.restorePending = function() {
  ...
  try {
    ...
    this._targetReps = p._targetReps || null; this._accumulatedReps = p._accumulatedReps || 0;
...
Training._renderActive = function() {
  ...
  // 目标次数与递减组 UI
  if (this._targetReps != null) {
    const pct = Math.min(100, (this._accumulatedReps / this._targetReps) * 100);
    html += `<div style="margin:8px 0">
      <div style="font-size:0.8rem;color:var(--text-secondary);margin-bottom:4px">🎯 累计: ${this._accumulatedReps} / ${this._targetReps}</div>
      <div style="height:8px;background:var(--bg-input);border-radius:4px;overflow:hidden">
        <div style="height:100%;width:${pct}%;background:var(--accent);border-radius:4px;transition:width 0.3s"></div>
      </div>
      ${this._accumulatedReps >= this._targetReps ? '<div style="font-size:0.8rem;color:var(--success);margin-top:2px">✅ 目标达成！</div>' : ''}
    </div>
    <label class="form-checkbox" style="margin-bottom:8px">
      <input type="checkbox" id="drop-set-checkbox">
      <span>🔽 递减续组（减重继续做）</span>
    </label>`;
  } else {
    html += `<div class="form-group" style="display:flex;gap:8px;align-items:flex-end">
      <div style="flex:1">
        <label class="form-label">🎯 目标次数（可选）</label>
        <input type="number" class="form-input" id="target-reps-input" placeholder="如 40" step="1">
      </div>
      <button type="button" class="btn btn-secondary" onclick="Training._setTargetReps()" style="white-space:nowrap">设置</button>
    </div>`;
  }
  ...
  if (this.setData.length > 0) {
    ...
    this.setData.forEach((s, i) => {
      const dropLabel = s._dropGroup ? ' <span style="color:#e68a2e;font-size:0.75rem">🔽递减</span>' : '';
      html += `<div style="font-size:0.85rem;padding:4px 0;border-bottom:1px solid var(--border)">组 ${i+1}: ${UI.renderSetData(s, ex.fields) || '—'}${dropLabel}</div>`;
    });
    html += '</div>';
  }
